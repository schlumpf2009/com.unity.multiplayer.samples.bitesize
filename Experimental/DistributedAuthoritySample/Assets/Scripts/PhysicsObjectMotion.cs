...
public partial class PhysicsObjectMotion : BaseObjectMotionHandler
{
    ...
    [Tooltip("The rate at which a remote collisio's force decays over time.")]
    [Range(0.05f, 0.95f)]
    public float RemoteCollisionForceDecay = 0.25f;

    [Range(0.001f, 10.0f)]
    public float AngularCollisionMultiplier = 1.0f;

    [Range(1.0f, 10.0f)]
    public float LinearCollisionMultiplier = 1.0f;

    public const float MaxMass = 5.0f;
    public const float MinMass = 0.10f;

    public MinMaxVector2Physics MinMaxStartnigTorque = new MinMaxVector2Physics(5.0f, 15.0f);
    public MinMaxVector2Physics MinMaxStartnigForce = new MinMaxVector2Physics(5.0f, 30.0f);
    ...
    public override void OnNetworkSpawn()
    {
        ...
        if (HasAuthority)
        {
            ...
            if (!BeenInitialized.Value)
            {
                ...
                var torque = GetRandomVector3(MinMaxStartnigTorque, Vector3.one, true);
                Rigidbody.AddTorque(torque, ForceMode.Impulse);
                UpdateTorque(torque);
                var force = GetRandomVector3(MinMaxStartnigForce, Vector3.one, true);
                force.y = 0f;
